    ...
    elseif num == 3
        ...
        for i = 1:1:dr
            ...
            for k = 1:k_num + m_num
                ...
                min_tt(k,end+1) = d_t_end;
                min_rr{k,end+1} = nroute_e;
                min_tt = min_tt - min(min_t);
                
            end
            value = max(max(min_tt));
            [row, col] = find(value == min_tt);
            %[~,mk] = min(min_t);
            InitialRoute{1,row(1)} = min_rr{row(1),col(1)};
        end
        newRoute = InitialRoute;
    end
end
